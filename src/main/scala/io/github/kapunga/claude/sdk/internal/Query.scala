...
final class Query private (
    ...
    for
      ...
      request = {
        ...
        config.agents.foreach { agents =>
          f += ("agents" -> Json.fromJsonObject(JsonObject.fromMap(agents.view.mapValues(Json.fromJsonObject).toMap)))
        }
  ...
  private def routeMessage(msg: JsonObject): IO[Unit] =
    ...
    msgType match
      ...
      case _ =>
        ...
        val trackResult =
          if msgType.contains("result") then firstResultDeferred.complete(()).void.handleError(_ => ())
          else IO.unit
                ...
                case Some(deferred) =>
                  ...
                  val result =
                    if isError then
                      ...
                      Right(response("response").flatMap(_.asObject).getOrElse(JsonObject.empty))
        ...
        config.canUseTool match
          ...
          case Some(canUseTool) =>
            ...
            canUseTool(toolName, input, context).flatMap { result =>
              val responseJson = io.circe.Encoder[PermissionResult].apply(result).asObject
          ...
          callbacks.get(callbackId) match
            ...
            case Some(callback) =>
              ...
              hookInputJson.as[HookInput](hookInputDecoder) match
                case Left(err) =>
                  sendControlResponse(requestId, Left(s"Failed to parse hook input: ${err.message}"))
                case Right(parsedInput) =>
                  val ctx = HookContext()
                  callback(parsedInput, toolUseId, ctx).flatMap { output =>
                    val outputJson = io.circe.Encoder[HookJsonOutput].apply(output).asObject
                      .getOrElse(JsonObject.empty)
    ...
    for
      ...
      result    <- deferred.get.timeoutTo(
                     ...
                     pendingResponses.update(_ - requestId) >>
                       IO.raiseError(new ControlProtocolError(
  ...
  private def sendControlResponse(requestId: String, result: Either[String, JsonObject]): IO[Unit] =
    val response = result match
      case Right(data) =>
        SDKControlResponse(ControlResponseData.Success(ControlSuccessResponse(requestId, Some(data))))
      case Left(error) =>
...
object Query:
  ...
  def make(
    ...
    for
      pendingResponses    <- Ref.of[IO, Map[String, Deferred[IO, Either[Throwable, JsonObject]]]](Map.empty)
    ...
    yield new Query(
      transport, config, pendingResponses, hookCallbacksRef,
      requestCounter, messageQueue, closedRef, initResultRef,
      firstResultDeferred, supervisor,
    )
  ...
  def resource(
    ...
    for
      pendingResponses    <- Resource.eval(Ref.of[IO, Map[String, Deferred[IO, Either[Throwable, JsonObject]]]](Map.empty))
    ...
    hooks.toList.traverse { case (event, matchers) =>
      matchers.traverse { matcher =>
        matcher.hooks.traverse { callback =>
          for
            id <- hookCallbacksRef.modify { cbs =>
